...
export default async function PublicationsPage() {
  const items = await wpApi.getPosts(1, undefined, 'rscn_publications');

  return (
    <PageHeader title="Publications" description="Scientific publications, reports and documents." titleContainerClassName="pageHeaderTitle" imageUrl="">
            ...
            {items.posts.map((item: any) => {
              ...
              return (
                <div key={item.id} className="bg-white rounded-xl border-2 flex flex-col items-center p-6 min-h-[370px]" style={{ borderColor: 'var(--theme-primary-light-text)' }}>
                  ...
                  <a href={`/publications/${item.slug}`} className="mt-auto text-sky-500 font-medium hover:underline" style={{ color: 'var(--napoli-blue)' }}>See More</a>
                </div>
              );
            })}
          </div>
